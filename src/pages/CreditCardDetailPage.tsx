...
import { listInstallmentPlansForCard } from '../api/installmentsApi'
...
export default function CreditCardDetailPage() {
  ...
  // Misma queryKey que InstallmentPlansSection: comparten la caché, no duplica la petición.
  const { data: installmentPlans } = useQuery({
    queryKey: ['installmentPlans', id],
    queryFn: () => listInstallmentPlansForCard(id!),
    enabled: Boolean(card),
  })
  ...
  const planTransactionIds = useMemo(
    () => new Set((installmentPlans ?? []).map((plan) => plan.transactionId)),
    [installmentPlans],
  )
            ...
            <Table size="small">
              ...
              <TableBody>
                {transactions.map((transaction: Transaction) => {
                  const isEditablePurchase = transaction.type === 'CREDIT_CARD_PURCHASE' && !planTransactionIds.has(transaction.id)
                  return (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell align="right">
                        {isEditablePurchase && (
                          <IconButton size="small" aria-label={t('common:actions.edit')} onClick={() => setEditingPurchase(transaction)}>
